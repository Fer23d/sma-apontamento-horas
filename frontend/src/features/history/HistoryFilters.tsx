...
export function HistoryFilters({ value, onChange, onApply }: { value: HistoryFiltersValue; onChange: (value: HistoryFiltersValue) => void; onApply: () => void }) {
  ...
  return (
    <form onSubmit={(event) => { event.preventDefault(); onApply() }} className="grid gap-4 rounded-2xl border ui-border ui-surface p-5 md:grid-cols-2 xl:grid-cols-4" aria-label="Filtros do histórico">
      ...
      <div><label htmlFor="history-client" className="text-sm font-bold ui-text">Cliente</label><input id="history-client" type="search" value={value.clientName} onChange={(event) => set('clientName', event.target.value)} className={fieldClassName} placeholder="Buscar pelo nome" /></div>
      <div><label htmlFor="history-project" className="text-sm font-bold ui-text">Número da contratada</label><input id="history-project" value={value.projectCode} onChange={(event) => set('projectCode', event.target.value)} className={fieldClassName} /></div>
      ...
      <div><label htmlFor="history-discipline" className="text-sm font-bold ui-text">Disciplina</label><select id="history-discipline" value={value.disciplineCode} onChange={(event) => set('disciplineCode', event.target.value as HistoryFiltersValue['disciplineCode'])} className={fieldClassName}><option value="">Todas</option><option value="—">Não se aplica</option><option value="A">Automação</option><option value="E">Elétrica</option><option value="G">Geral</option><option value="M">Mecânica</option></select></div>
