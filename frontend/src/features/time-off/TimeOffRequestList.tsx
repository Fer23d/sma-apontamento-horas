        ...
        return (
          <article key={request.id} className="rounded-2xl border ui-border ui-surface p-5 shadow-sm">
            <div className="flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between">
              <div><div className="flex flex-wrap items-center gap-2"><h3 className="font-extrabold ui-heading">{formatDatePtBr(request.date)}</h3><span className="rounded-full border ui-border px-2.5 py-1 text-xs font-bold">{statusLabels[request.status]}</span></div><p className="mt-2 text-sm ui-text">{request.reason}</p><p className="mt-2 text-xs ui-text-subtle">Squad registrada: {request.assignmentSnapshot?.squadName ?? 'Não disponível'} · Supervisor: {request.assignmentSnapshot?.supervisorName ?? 'Não disponível'}</p>{request.cancellationReason && <p className="mt-2 text-xs ui-text-subtle">Motivo do cancelamento: {request.cancellationReason}</p>}</div>
