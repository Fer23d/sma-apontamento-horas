...
export function Sidebar({ isOpen, onNavigate, onKeyDown }: SidebarProps) {
  ...
  return (
    <aside
      ...
      data-drawer-panel
      onKeyDown={onKeyDown}
      className={`${isOpen ? 'visible translate-x-0' : 'invisible -translate-x-full'} fixed bottom-0 left-0 top-20 z-30 flex w-72 min-w-0 flex-col overflow-y-auto bg-[var(--sma-shell-sidebar)] text-white shadow-2xl transition-[transform,visibility] duration-200 lg:visible lg:static lg:h-[calc(100vh-5rem)] lg:translate-x-0 lg:self-start lg:shadow-none`}
    >
      <div className="flex items-center justify-between border-b border-white/10 px-4 py-3 lg:hidden">
        ...
        <button type="button" onClick={onNavigate} data-drawer-initial-focus className="rounded-lg border border-white/20 px-3 py-2 text-sm font-bold" aria-label="Fechar navegação lateral">Fechar</button>
      </div>
        ...
        <section className="border-b border-white/10 p-4" aria-label="Perfil atual do colaborador">
          <div className="flex items-center gap-3">
            ...
            <div className="min-w-0">
              <p className="truncate text-sm font-extrabold">{profile.name}</p>
              <p className="truncate text-xs text-slate-300">{profile.jobTitle}</p>
            </div>
          </div>
          <div className="mt-4 flex items-center gap-2 rounded-xl bg-white/10 p-3">
            ...
            <div className="min-w-0">
              ...
              <p className="truncate text-xs font-bold">{assignment?.squadName ?? 'Não definida'}</p>
            </div>
          </div>
        </section>
      )}
