...
function statusLabel(status: ReportStatus) { return status === 'PENDING' ? 'Pendente' : status === 'APPROVED' ? 'Aprovado' : status === 'REJECTED' ? 'Rejeitado' : status === 'CANCELLED' ? 'Cancelado' : 'Ativo' }
function flattenReportRows(supervisors: ReportSupervisor[]): DirectorateReportRow[] {
  return supervisors.flatMap((supervisor) => supervisor.collaborators.flatMap((collaborator) => collaborator.entries.map((entry) => ({
    supervisor: supervisor.name,
    colaborador: collaborator.name,
    dataApontamento: entry.entryDate,
    horasTrabalhadas: formatMinutes(entry.durationMinutes),
    horasExtras: formatMinutes(entry.extraMinutes),
    horasNoturnas: formatMinutes(entry.nightMinutes),
    bancoDeHoras: formatSignedMinutes(collaborator.bankMinutes),
...
function DiretoriaSidebar({ onSignOut }: { onSignOut: () => void }) {
  ...
  return <aside className="hidden h-[calc(100vh-5rem)] w-64 flex-col bg-[var(--color-sidebar)] text-[var(--color-sidebar-text)] lg:sticky lg:top-20 lg:flex lg:self-start"><section className="border-b border-[var(--color-sidebar-border)] p-4"><div className="flex items-center gap-3"><span className="flex h-12 w-12 items-center justify-center rounded-full border border-[var(--color-sidebar-border)] bg-[var(--color-sidebar-surface)] text-sm font-extrabold">DI</span><div><p className="text-sm font-extrabold leading-tight">Diretoria SM&A</p><p className="mt-0.5 text-xs leading-tight text-[var(--color-sidebar-text-muted)]">Visão macro</p></div></div></section><nav className="flex-1 space-y-2 p-4" aria-label="Menu lateral da diretoria"><NavLink to="/administracao" end className={({ isActive }) => `${linkClass({ isActive })} tour-painel-diretor`}><span className="flex h-8 w-8 items-center justify-center rounded-lg bg-[var(--color-sidebar-surface)] text-xs">DI</span><span className="flex-1">Painel Diretor</span></NavLink><NavLink to="/administracao/equipes" className={linkClass}><span className="flex h-8 w-8 items-center justify-center rounded-lg bg-[var(--color-sidebar-surface)] text-xs">EQ</span><span className="flex-1">Equipes</span></NavLink><NavLink to="/administracao/relatorios" className={({ isActive }) => `${linkClass({ isActive })} tour-relatorios`}><span className="flex h-8 w-8 items-center justify-center rounded-lg bg-[var(--color-navigation-active-detail)] text-xs text-[var(--color-primary)]">RE</span><span className="flex-1">Relatórios</span></NavLink><NavLink to="/administracao?view=avisos" className={linkClass}><span className="flex h-8 w-8 items-center justify-center rounded-lg bg-[var(--color-sidebar-surface)] text-xs">AV</span><span className="flex-1">Avisos</span></NavLink></nav><div className="border-t border-[var(--color-sidebar-border)] p-4"><button type="button" onClick={onSignOut} className="w-full rounded-xl border border-[var(--color-sidebar-border)] px-4 py-3 text-left text-sm font-bold text-[var(--color-sidebar-text)] hover:bg-[var(--color-navigation-hover)]">Sair do sistema</button></div></aside>
}
...
export function RelatoriosPage() {
  ...
  const exportRows = useMemo(() => flattenReportRows(supervisors), [supervisors])
  const handleExport = async () => {
    if (!validRange || isExporting) return
    setIsExporting(true)
    setExportError(null)
    try {
      await exportDirectorateReport(exportRows, { startDate, endDate })
    } catch (error) {
