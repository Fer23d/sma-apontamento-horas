  ...
  return (
    <PageContainer title="Folgas" description="Solicite folgas futuras e acompanhe o encaminhamento para a supervisão da sua squad." contained={false}>
      <div className="space-y-6">
        ...
        {controller.isLoading ? <p aria-live="polite" className="rounded-2xl bg-white p-8 text-center dark:bg-slate-900">Carregando folgas…</p> : <TimeOffRequestList requests={controller.requests} today={controller.today} onRemovePending={setRemoveTarget} onCancelApproved={setCancelTarget} />}
      </div>
      ...
      <ConfirmDialog open={Boolean(cancelTarget)} title="Cancelar folga aprovada?" description="O saldo projetado será recalculado e a supervisão será notificada." confirmLabel="Cancelar folga" onCancel={() => { setCancelTarget(null); setCancelReason('') }} onConfirm={() => { if (cancelTarget && cancelReason.trim()) void controller.cancelApproved(cancelTarget, cancelReason).then(() => { setCancelTarget(null); setCancelReason('') }) }}>
        <label htmlFor="time-off-cancel-reason" className="text-sm font-bold text-slate-800 dark:text-slate-200">Motivo do cancelamento</label><textarea id="time-off-cancel-reason" rows={3} value={cancelReason} onChange={(event) => setCancelReason(event.target.value)} className={fieldClassName} />
      </ConfirmDialog>
    </PageContainer>
  )
}
